...
基于 **C++17** 和 **EasyX** 图形库的 Windows 桌面图表可视化工具。加载 CSV 数据，以柱状图、饼图、折线图、面积图四种形式呈现，支持数据排序、统计分析和 PNG 导出。

## 快速开始

```bash
# 编译（需要 MinGW-w64 + EasyX）
g++ -std=c++17 -Isrc -ID:\easyx4mingw_25.9.10\include \
    -LD:\easyx4mingw_25.9.10\lib64 \
...
> 或者直接用 Code::Blocks 打开 `ChartSystem.cbp`，按 F9 编译运行。
...
│   │   ├── BarChart.h / .cpp       #   柱状图
│   │   ├── PieChart.h / .cpp       #   饼图
│   │   ├── LineChart.h / .cpp      #   折线图（含 PlotCoords）
...
│   │   └── Pages.h / .cpp          #   Page 基类 + MainPage + ChartPage
│   └── utils/
...
├── data/                           # 示例数据（5 个 CSV）
├── example/                        # 导出示例（PNG 截图）
├── ChartSystem.cbp                 # Code::Blocks 工程文件
...
├── MainPage     主页：4 张卡片 + 输入框 + 7 个按钮
└── ChartPage    图表页：顶栏 + 统计卡片 + 图表区 + 7 个按钮
```

### 设计要点

- **容器化事件分发**：`Page` 基类持有 `vector<Widget*>`，事件（MouseMove/Down/Up/KeyDown）遍历分发，子类只覆盖 `draw()`。新增控件只需 `push_back`。
- **Card 继承复用**：`TextInput` 和 `PopupCard` 继承 `Card`，背景和阴影由基类绘制，子类仅处理自身逻辑。
...
- **MinGW-w64 (g++ ≥ 8.0)** — C++17 支持
- **[EasyX](https://easyx.cn/)** — Windows 轻量图形库
- **Windows 7+** — Win32 API
- **GDI32 / OLE32** — 系统图形与 COM 库
